...
class _AddItemDialogState extends State<AddItemDialog> {
  ...
  void initState() {
    ...
    if (widget.isEdit && widget.initialItem != null) {
      ...
      final autoName = _computeAutoGeneratedName().trim().toLowerCase();
      _userHasCustomName = item.name.trim().toLowerCase() != autoName;

      if (item.hsn.isNotEmpty) {
        _validateHsn();
      }
    } else {
      // PREFILL FROM SCANNED DATA
      if (widget.initialName != null && widget.initialName!.trim().isNotEmpty) {
        ...
        _userHasCustomName = false;
      }

      _hsnController.text = widget.initialHsn ?? '';
      _unitController.text = (widget.initialUnit != null && widget.initialUnit!.isNotEmpty)
          ? widget.initialUnit!.toUpperCase()
          : '';

      final rate = widget.initialGstRate ?? 18.0;
      final matchedCategory = _taxCategories.firstWhere(
        (c) => c.contains('${rate.toInt()}%'),
        orElse: () => 'GST ${rate.toInt()}%',
      );
      _selectedTaxCategory = matchedCategory;
      _taxCategoryController.text = matchedCategory;

      if (widget.initialPrice != null && widget.initialPrice! > 0) {
        _purchasePriceController.text = widget.initialPrice!.toStringAsFixed(2);
        _salesPriceController.text = widget.initialPrice!.toStringAsFixed(2);
      }

      if (_hsnController.text.isNotEmpty) {
        WidgetsBinding.instance.addPostFrameCallback((_) => _validateHsn());
      }
    }

    _hsnController.addListener(_onIdentificationFieldsChanged);
    _unitController.addListener(_onIdentificationFieldsChanged);
    _taxCategoryController.addListener(_onIdentificationFieldsChanged);
  ...
  void _onNameFieldChanged() {
    if (_nameFocusNode.hasFocus) {
      ...
      final auto = _computeAutoGeneratedName().trim();
      if (current.isEmpty) {
        _userHasCustomName = false;
  ...
  void _onIdentificationFieldsChanged() {
    if (!_userHasCustomName) {
      _nameController.text = _computeAutoGeneratedName();
    }
  }
  ...
  void _enforceValidUnitSelection() {
    ...
    if (current.isEmpty) {
      _onIdentificationFieldsChanged();
      return;
    }
  ...
  void _enforceValidTaxCategorySelection() {
    ...
    if (_taxCategoryController.text != match) {
  ...
  String _computeAutoGeneratedName() {
    final hsn = _hsnController.text.trim();
    final taxCategory = _selectedTaxCategory.isNotEmpty
        ? _selectedTaxCategory
        : _taxCategoryController.text.trim();
    final tax = _extractTaxPercentage(taxCategory);
    final unit = _unitController.text.trim().toUpperCase();

    final parts = <String>[];
    if (hsn.isNotEmpty) parts.add(hsn);
    if (tax.isNotEmpty) parts.add(tax);
          ...
          if (widget.isEdit && originalName != null && originalName.isNotEmpty) {
            ...
            targetIdx = itemsList.indexWhere((i) {
              final n = (i['name'] ?? i['itemName'] ?? '')
                  ...
                  .trim()
                  .toLowerCase();
              return n == enteredName.toLowerCase();
            });
          }
  ...
  @override
  void dispose() {
    _hsnController.removeListener(_onIdentificationFieldsChanged);
    _unitController.removeListener(_onIdentificationFieldsChanged);
    _taxCategoryController.removeListener(_onIdentificationFieldsChanged);
