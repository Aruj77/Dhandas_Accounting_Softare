...
class _VoucherManageListScreenState extends State<VoucherManageListScreen> {
  ...
  Future<void> _confirmAndDelete(List<Map<String, dynamic>> toDelete) async {
    ...
    if ((shouldDelete ?? false) && mounted) {
      ...
      final fy = (widget.company['activeFinancialYear'] ?? AppDateUtils.defaultFinancialYear).toString();
      if (folderPath != null) {
        await StorageService.saveAllVouchers(folderPath: folderPath, financialYear: fy, voucherType: widget.voucherType, vouchers: _vouchers);
      }
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isPlural ? '${toDelete.length} vouchers deleted.' : 'Voucher deleted.'), backgroundColor: AppColors.error, behavior: SnackBarBehavior.floating));
    }
  }
