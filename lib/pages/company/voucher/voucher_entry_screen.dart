...
class _VoucherEntryScreenState extends State<VoucherEntryScreen> {
  ...
  bool get _isViewingExistingVoucher =>
      _isEditingExisting || _sessionIndex != -1;

  Color get _themeColor =>
      _isSalesVoucher ? AppColors.warning : AppColors.primary;
  Color get _screenBg =>
  ...
  Future<void> _requestExit() async {
    ...
    final shouldExit = await VoucherDialogUtils.showUnsavedChangesDialog(
      context,
    );
  ...
  Future<void> _loadCompanyMastersOnly() async {
    await LoadingService.wrap(() async {
      ...
      _availableSeries
        ..clear()
        ..addAll(
          (rawMasters['series'] as List? ?? ['Main'])
  ...
  void _attachItemRowListeners(VoucherItemRow row) {
    row.itemFocus.addListener(() {
      ...
      final exists = _itemsMasterList.any(
        (i) => i.name.trim().equalsIgnoreCase(text),
      );
      if (!exists) {
  ...
  Future<void> _checkVoucherNumberOnBlur() async {
    ...
    final fy =
        (widget.company['activeFinancialYear'] ??
                AppDateUtils.defaultFinancialYear)
            .toString();
    if (folderPath == null) return;
    ...
    final currentId =
        _activeEditingVoucher?['id']?.toString() ??
        widget.voucherToEdit?['id']?.toString();
    final match = existingVouchers.where((v) {
      final matchesNo =
          (v['voucherNumber'] ?? '').toString().trim().toLowerCase() ==
          vchText.toLowerCase();
      final isSelf =
    ...
    if (match != null && mounted) {
      ...
      final assignedParty = GstPartyUtils.extractPartyName(
        (match['party'] ?? '').toString(),
      );
  ...
  void _attachControllerListeners() {
    ...
    _partyFocus.addListener(() {
      ...
      final exists = _currentAvailableParties.any(
        (p) =>
            p.displayName.trim().equalsIgnoreCase(text) ||
            p.name.trim().equalsIgnoreCase(text),
      );
      if (!exists) {
        _isHandlingMasterNotFound = true;
        VoucherDialogUtils.showMasterNotFoundDialog(
  ...
  void _handleManualSaleTypeChange() {
    ...
    if (partyState.isNotEmpty) {
      ...
      final mismatch =
          (partyIsInterstate && isExplicitLocal) ||
  ...
  bool _parseAndValidateDate() {
    ...
    if (parsed == null ||
        parsed.isBefore(_fyStartDate) ||
        parsed.isAfter(_fyEndDate)) {
      setState(
        () => _dateError = parsed == null ? 'Invalid date' : 'Date outside FY',
      );
      return false;
    }
  ...
  String _getCompanyStateCode() {
    final rawGstin =
        (widget.company['gstin'] ?? widget.company['gstNumber'] ?? '')
            .toString()
            .trim();
  ...
  String _extractPartyStateCode(String partyText) {
    ...
    final matched = _currentAvailableParties
        .where(
          (p) =>
              p.name.equalsIgnoreCase(clean) ||
              p.displayName.equalsIgnoreCase(clean),
        )
        .firstOrNull;
  ...
  bool _handleAltE() {
    return VoucherMasterActions.handleAltE(
        ...
        setState(() {
          void updateList(List<PartyMasterModel> list) {
            final i = list.indexWhere(
              (p) => p.name.equalsIgnoreCase(updated.name),
            );
      ...
      onItemUpdated: (i, updated) {
        setState(() {
          final idx = _itemsMasterList.indexWhere(
            (m) => m.name.equalsIgnoreCase(updated.name),
          );
          if (idx != -1) _itemsMasterList[idx] = updated;
          for (final r in _items.where(
            (r) => r.item.text.equalsIgnoreCase(updated.name),
          )) {
  ...
  Map<String, dynamic> _buildCurrentVoucherPayload() {
    ...
    return {
          ...
          .map(
            (i) => {
              ...
              'hsn': i.hsn.isNotEmpty
                  ...
                  : (_itemsMasterList
                        .firstWhere(
                          (m) => m.name.equalsIgnoreCase(i.item.text),
                          orElse: () => ItemMasterModel.empty(),
                        )
                        .hsn),
              'qty': i.qty.text,
      ...
      'createdAt':
          _activeEditingVoucher?['createdAt'] ??
  ...
  Future<void> _saveVoucher() async {
    ...
    if (partyText.isEmpty ||
        !_currentAvailableParties.any(
          (p) => p.displayName == partyText || p.name == partyText,
        )) {
      return _showValidationError(
        'Select a valid registered Party Ledger.',
        _partyFocus,
      );
    }
    ...
    if (validItems.isEmpty) {
      _showValidationError(
        'Add at least one item with Qty & Amount > 0.',
        null,
      );
  ...
  Future<void> _executeVoucherPersistence() async {
    ...
    if (_isSalesVoucher) {
      final shouldPrint = await showDialog<bool>(
        context: context,
        builder: (ctx) =>
            _PrintStudioConfirmDialog(vchNo: _vchNoController.text),
      );
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      TextButton.icon(
                        onPressed: () => _openCalculatorForController(
