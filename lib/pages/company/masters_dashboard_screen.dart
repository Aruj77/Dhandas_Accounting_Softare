...
class _MasterActionsSheet extends StatelessWidget {
  ...
  static const _actions = [
    (action: MasterAction.add, title: 'Add New', desc: 'Create and configure a new master record', icon: Icons.add_circle_outline_rounded, color: AppColors.success),
    (action: MasterAction.modify, title: 'Modify', desc: 'View, edit, or delete existing records', icon: Icons.edit_note_rounded, color: AppColors.primaryAccent),
  ];
    ...
    return Container(
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            Container(width: 40, height: 4, decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(20))),
            const SizedBox(height: 18),
            Text(item.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: AppColors.textPrimary)),
                    ...
                    child: Row(
                      children: [
                        Container(
                          width: 40,
                          height: 40,
                          decoration: BoxDecoration(color: a.color.withValues(alpha: .1), borderRadius: BorderRadius.circular(12)),
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(a.title, style: const TextStyle(fontSize: 13.5, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                              const SizedBox(height: 2),
                              Text(a.desc, style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                            ],
                          ),
                        ),
  ...
  Future<void> _deleteRecord(Map<String, dynamic> record) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Confirm Delete', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
        content: Text('Are you sure you want to delete "$title"? This action cannot be undone.', style: const TextStyle(color: AppColors.textSecondary)),
        actions: [
          OutlinedButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
          ElevatedButton(
            onPressed: () => Navigator.pop(ctx, true),
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.error),
            child: const Text('Delete', style: TextStyle(color: AppColors.surface, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );

    if (confirm != true || _folderPath == null) return;
